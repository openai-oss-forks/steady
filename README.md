...
This source-only snapshot does not publish npm or JSR packages. Check out a
reviewed full commit SHA, install Deno 2.7.11, then run from the repository:

```bash
./scripts/bootstrap
./scripts/start /absolute/path/to/api.yaml --host 127.0.0.1
```

The examples below use `steady` as shorthand for `./scripts/start`. GitHub
Actions and publishing integrations are intentionally absent. Local unit tests,
lint, and synthetic-spec fuzz tests remain available.

### Safety limits

Use trusted local test specifications and synthetic credentials. Steady is not
an authenticated service and must not be exposed through a proxy or tunnel. TCP
listeners are restricted to `127.0.0.1` and `::1`; Unix sockets remain available
for local tests.

Request bodies are limited to 16 MiB and 30 seconds. Library callers can set
`maxRequestBodyBytes` explicitly, up to 1 GiB. Response generation rejects
arrays over 1,000 items, strings over 65,536 characters, and excessive nested
expansion, including literal defaults and examples. Encoded responses and
streams are limited to 16 MiB. Streams generate on reader demand and allow at
most 1,000 events and a 10-second per-event interval. The server retains at most
10,000 runtime diagnostics and 100 sessions with 1,000 diagnostics each; reports
indicate when diagnostics were omitted.

All log formats redact header values, query strings, bodies, and request-derived
diagnostic prose, including at full verbosity. Logs retain diagnostic codes,
categories, schema pointers, and route templates. Detailed validation responses
remain available to the local caller; do not use real credentials or customer
data in tests.
