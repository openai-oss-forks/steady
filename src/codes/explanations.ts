...
const EXPLANATIONS: Record<ECode, Explanation> = {
  E1023: {
    description: "The mock response could not be generated or streamed.",
    reasoning:
      "The selected response specification could not be served successfully.",
    example: "A response stream fails while generating its next item.",
    fix: "Check the selected response schema and synthetic examples.",
    seeAlso: ["E3024"],
  },
  E3024: {
    description: "A request or response exceeded a runtime safety limit.",
    reasoning:
      "Steady rejects excessive resource use instead of silently truncating the result.",
    example: "A request body exceeds its configured byte limit.",
    fix:
      "Reduce the request, generation settings, or response example. Review the documented safety limits.",
    seeAlso: ["E1023"],
  },
  // ── E1xxx: Spec Issues ──────────────────────────────────────────────
