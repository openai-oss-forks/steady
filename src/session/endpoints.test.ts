...
Deno.test("handleSessionRequest", async (t) => {
  ...
  await t.step("returns session report as JSON", async () => {
    ...
    assertEquals(body.sdk_issues[0].path, "[unmatched route]");
  ...
  await t.step("includes attribution in each diagnostic", async () => {
    ...
    assertEquals(issue.attribution.reasoning.length, 0);
    assertEquals(issue.suggestion, "Run steady explain E3007");
  });
});
